...
def infer_target_type(target: str, explicit_type: str = '') -> str:
    explicit_type = str(explicit_type or '').strip().lower()
    if explicit_type in {'action', 'dsl', 'pattern_dsl'}:
...
def score_stage2_completion(
        ...
        graph_samplers,
        graph_split: str = 'train') -> dict:
    resolved_type = infer_target_type(target, target_type)
    if resolved_type == 'action':
        return score_action_completion(
...
def score_stage2_completion_batch(
        completions,
        targets,
        sources,
        condition_texts,
    ...
    return [
        ...
        for completion, target, source, condition_text, target_type in zip(
            completions,
            targets,
            sources,
            condition_texts,
